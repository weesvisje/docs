...
1. Locate the __Info.plist__ file and open it
...
When Visual Studio generated your project, there should be a file called __AppDelegate.cs__. We will need an instance of Ditto throughout this tutorial and the app's lifecycle.
...
4. Construct an instance of Ditto with an online playground identity using the APP ID and Playground Token of the app that you just created on the portal. We are using an `.OnlinePlaygroundV2` setup, which should suffice for this tutorial. However, you should never deploy this to a production environment.
5. We want to enable all peer to peer transport configurations
6. We will call `tryStartSync`. This method can throw an error in the event that the license token is invalid or expired.

```swift title=AppDelegate.cs
using Foundation;
using UIKit;
using System;
...
Create a new cs file called __Task.cs in your project.
...
6. Select the Root View Controller top bar. Then go to the Inspectors panel and select the `identity` inspector. We will create a custom class for this View Controller. In the Class section type "TasksTableViewController".

 ![](TasksTableViewController.png)
 
7. Next, select the `Prototype cells` and give it an identifier of `taskCell`

![](taskCell.png)

9. Save that file in Xcode, then open the poject up in Visual Studio again. At this point, Visual Studio should have auto generated two files for you. A `TasksTableViewController.cs` file and a `TasksTableViewController.designer.cs` file. These are the class files that were created from the TasksTableViewController we created in Xcode.
10. Open the project up in Xcode again and you should now see two new files added to the project directory. A TasksTableViewController `h` file and a TasksTableViewController `m` file.
11. Open the Main.storyboard file. While pressing the `control` button on your keyboard select the button and drag under the `@interface SceneDelegate : UIResponder {
}` code inside the TasksTableViewController `h` file. Fill the information as follows:
  - Connection: Action
