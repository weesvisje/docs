...
Ditto's sync system is query-based, which means, that by default the SDK will
not sync data with other devices. Instead, the app creates query-based
observations that define which data it wants to sync. When the device is
observing a query, then other devices will share data matching that query
with it.
