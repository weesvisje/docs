...
When Ditto sends an HTTP POST request to your server-side authentication
webhook, your code should respond with the permissions granted to that user.

This response is a special format that can be parsed by Ditto peers and accept
or reject reads and writes from individual clients. 

:::info
This page is a reference for writing Ditto permissions for the
