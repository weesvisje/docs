...
Maps are queriable as well, so you can safely sort objects using the [query api](/concepts/querying).
...
replicate changes to a very long json blob. Using the Map type allows you to make changes to just a single
...
The following `upsert` call will only change the property "color: red"  to
"color: blue". 
...
Another device could simultaneously update the mileage, and both updates will converge.
