...
from the internet. Writes from all peers will syncronize and Ditto will resolve
any conflicts.
...
## Upserting 
 ...
 possible that the current device just hasn't syncronized that document yet.
...
Updating an existing document is different for three different 
...
single database call, that will be syncronized at the same time to other
...
:::warning
You cannot combine delete and update in the same write transaction.
:::
