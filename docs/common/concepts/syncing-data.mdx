...
Devices should periodically restart the query with a new date. Notice that the
...
### 3. Properly Stop Subscription Before Evicting

When working with eviction and subscription it is important to remember that their queries should never overlap. This is because if you try to evict data that you are also subscribed to, then after you evict the data you will immediately sync it back to the device since the subscription is still syncing that data. To illustrate this we can use the same time-based syncing example that we used above. In this example we only want to sync/store data on the device that was created within the past 24 hours. Any data older than 24 hours should be removed from the device. Also, after 24 hours have passed we will need to update our subscription to make sure it is syncing the new correct 24 hour time frame.
...
3. Stop subscription 
```
Call cancel() to stop a subscription 
subscription.cancel()
```
...
*Your subscription variable should remain in scope throughout the runtime time of the app, so that it will continue to sync data and not be garbage collected. 

Though there are different ways you may stop a subscription (setting it to null, reassigning its value) it is important to always call `cancel` on the subscription. 

Subscriptions run asynchronously, which means that if you want to stop a subscription and evict the data it was syncing at the same time it is important to ensure your subscription has completely stopped before `evict` is called or else you may experience a race condition where the evict call happens before the subscription is stopped. The way to ensure your subscription completely stops before `evict` is called is by calling `cancel` on the subscription.
```
// Start a subscription for syncing only red cars
subscription = ditto.store.collection("Cars").find("color == red").subscribe()

// Stop subscription using cancel()
subscription.cancel()

// Evict all red cars
ditto.store.collection("Cars").find("color == red").evict()
```
