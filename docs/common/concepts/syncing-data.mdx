...
To enable Ditto to sync data, you'll need to call `ditto.startSync()`. 

Preferably, you should call `startSync()` early on in your application's life cycle like in your `AppDelegate.application(_:didFinishLaunchingWithOptions:)` or `Application.onCreate` methods. Your application only needs to call this function once.
...
past 48 hours. At the end of that 48 hours, documents older than 48 hours are evicted and a
...
We want to only hold data on the device that was created in the past 48 hours.
Anything older than 48 hours should be removed locally from the device. However,
we don't want to remove this data from the entire mesh network, because we may
want to do analytics on it, with the Cloud Big Peer.

If the app is used on 2,000 flights per day, you would see a maximum of
4,000 documents locally on the device. If each document is 5Kb, the device could
replicate up to ~20 megabytes to the local database. 

```swift
ditto.store.collection("flights").find("createdAt > $args.yesterday", args: [
  "yesterday": Date().adding(hr: -48),
]).subscribe()
```

Once a day, the device evicts the data locally to free up storage, and
restarts the query with a new date.
...
When the application starts, the device in the kitchen only wants to syncronize orders that not
filled and not cancelled, and evict all orders that are cancelled or filled.
