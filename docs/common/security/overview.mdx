...
Most applications need a way to control which users can write or read certain data. Ditto security is controlled by __identities__. An identity is a combination of a `userID` and a set of `permissions` that identifies how a Ditto instance behaves with other ditto instances. 
...
via a globally unique AppId and may not be used across multiple Ditto apps.
