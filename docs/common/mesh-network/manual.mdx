...
```
ditto = Ditto(identity: .onlinePlayground(appID: "Your_AppId", token: "Your_Token", enableDittoCloudSync: true)) // Set to false to disable syncing with the cloud
```
...
You can also configure your Ditto instance to use websockets using a similar setup to the tcpServer above.
```
val transportConfig = DittoTransportConfig()
transportConfig.connect.websocketURLs.add()
