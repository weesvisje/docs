 ...
 */
 ...
 import React, { useState, cloneElement, Children, ReactElement, useEffect } from "react";
...
interface TabProps extends Props { 
 ...
 function Tabs(props: TabProps): JSX.Element {
   const {
     lazy,
     platform,
     block,
   ...
   useEffect(() => {
    if (groupId != null) {
      ...
      if (
        ...
        if (values.some((value) => value.value === relevantTabGroupChoice)) {
   ...
   }, [platform, tabGroupChoices[groupId]])
   ...
   const showTabs = values.length > 1 
   return (
